...
Item {
    ...
    // Raw appearance inputs are public so SkinScope can reuse the current user
    // preference while changing only its local design language.
    // 原始外观输入公开给 SkinScope，使它仅替换局部设计语言而保留用户明暗、字体和强调色。
    readonly property string uiFontFamily: ThemeManager ? ThemeManager.fontFamily : "Microsoft YaHei UI, Segoe UI Variable, Segoe UI, -apple-system, PingFang SC, Roboto, Noto Sans CJK SC, sans-serif"
    ...
    readonly property color rawAccentColor: ThemeManager ? ThemeManager.accentColor : _constants.accentDefaults.accent
    readonly property color rawAccentColorLight: ThemeManager ? ThemeManager.accentColorLight : _constants.accentDefaults.accentLight
    readonly property color rawAccentColorDark: ThemeManager ? ThemeManager.accentColorDark : _constants.accentDefaults.accentDark
    ...
    readonly property color accentColor: isNeobrutalism ? _constants.neoColors.primary
        ...
        : (isNeumorphism ? _constants.neumorphismColors.primary : rawAccentColor))
