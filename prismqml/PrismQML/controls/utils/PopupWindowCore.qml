...
Item {
    ...
    // Settle a queued prewarm before a press, so the released click opens warm
    // instead of building the native surface inside the click callback.
    // 在按下前结算排队中的预热，使抬起后的点击走暖路径。
    function flushQueuedPrewarm() {
        PopupPrewarm.flushQueuedPrewarm(
