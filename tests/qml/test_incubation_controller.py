...
不变量, 它们是修复生效的前提:
  1. App 创建后, engine 已装 PrismIncubationController(自动安装未被删)。
...
from prismqml.python.core.incubation import (  # noqa: E402
    PrismIncubationController, install_incubation_controller)
...
def main():
    ...
    # 不变量1: install 后 engine 装上 PrismIncubationController + 幂等
    engine = QQmlApplicationEngine()
    register_types(engine)
    c1 = install_incubation_controller(engine)
    if not isinstance(engine.incubationController(), PrismIncubationController):
        failures.append("install 后 engine 未装 PrismIncubationController")
