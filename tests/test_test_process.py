...
def test_runner_child_transitions_from_launcher_to_test_error_policy():
    code = """
import ctypes
import json
import sys
...
sys.stdout.write(json.dumps({"before": before, "after": state()}))
"""
    ...
    assert (
        state["before"]["error_mode"] & WINDOWS_LAUNCHER_ERROR_MODE_FLAGS
        == WINDOWS_LAUNCHER_ERROR_MODE_FLAGS
    )
...
def test_runner_preserves_windows_ntstatus_without_crashing():
    status = 0xC0000374
    code = f"""
import ctypes
...
def test_runner_timeout_terminates_child_process_tree():
    code = """
import subprocess
import sys
import time
...
"""
    ...
    assert not _windows_process_exists(child_pid)
