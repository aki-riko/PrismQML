...
2. **验证**：发布前必须通过统一零交互门禁；自动测试禁止直接启动
   ...
   - 🔴 **当前优化基线：probe 应退出码 0，且约 `174 OK / 0 错误 / 7 跳过 = 181`**。5 个 singleton（Enums / Translator / DpiManager / NotificationManager / PopupUtils）必须通过 QtObject wrapper 触发 QML 引擎真实创建并读取；仅允许跳过 7 个 required-property 内部子模块（ButtonContent / ButtonDropdown / ButtonProgress / ListWidgetItem / SettingsCardContent / HorizontalScrollMixin / ViewportMixin，由父组件注入 required property，单独 createComponent 不成立）。
