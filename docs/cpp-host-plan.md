...
- 🟡 **阶段 7 移动端（apk 构建与 emulator 验证已完成，控件级触摸细化进行中）**：
  ...
  - ✅ **Windows D3D11 滚动验收**：可见窗口新进程连续 3 次完成滚轮、原生拖拽和
    拖拽后滚轮交接；实际图形后端均为 Direct3D11，三个阶段均无超过 20ms 的帧间隔。

**性能评估**：`Enums.qml` 的 45 个模块已用 5 个全新进程样本测量，较最小 QtQuick
对象约增加 20ms QML 加载、17ms 对象创建。由于 Loader 会让现有别名在首轮 binding
期间为空，当前不做行为不兼容的懒加载改造，待 API 兼容的懒代理方案出现后再评估。
...
| `window.addPage(interface, icon, text, position="top", selectedIcon="", selectable=True, visible=True)` | `window.addPage(QString pageQmlUrl, QString icon, QString text, Position=Top, bool selectable=true, bool visible=true)` | `visible=false` 保留页面索引但不呈现导航项；⚠️ Python 的 `interface` 可传「类（懒加载）或实例」；C++ 无鸭子类型 → 用 `std::function<QObject*()>` 工厂 或 `QQmlComponent*` 对等懒加载 |
