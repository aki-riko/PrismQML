...
默认 FastSplash 会等待首页内容就绪、首屏加载指示退场，并确认主窗口随后提交了
一帧，才开始原有揭幕动画。仅窗口外壳完成不再提前交接，因此首页加载较慢时
启动画面会多停留相应时间，避免进入主页后再次显示“加载中…”。此条件只作用于
启动交接，普通页面切换继续使用原有懒加载过渡。
若首屏加载失败或始终未就绪，仍按现有 `Enums.duration.splashTimeout` 超时策略
交接主窗口，并输出警告，避免启动画面无限遮挡窗口。
