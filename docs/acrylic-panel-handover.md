...
## 2026-09-20：完整窗口复核——剩余方形外沿来自独立阴影

用户在 `8a9a0188` 后完整重启 `examples/main.py`，仍能看到同一条方形外沿。
此前的亚克力裁剪仅解决了一个子层，不能作为用户问题已解决的证据：独立
`NavigationView` 测试没有创建 `WindowsSplit` 旁边的 `NavigationPanelShadow`。

本轮加载完整 Gallery 与设置页，分别关闭亚克力、底色画布、导航描边、窗口
外框和导航阴影。只有关闭导航阴影，右侧贯穿全高的方形外沿才消失。
对应源码是 `NavigationPanelShadow` 的三个组合条件：阴影源上下各扩展一个
模糊半径、阴影圆角为零、从面板最右侧开始直线裁剪。面板已收圆，阴影仍为
长方形，两者之间露出页面底色，形成“圆角下方还有一块方形底板”的错觉。

修正范围仅为阴影轮廓：源高度与可见窗口一致，右侧采用与面板相同的圆角；
绘制区域从圆弧起点开始，通过反向轮廓遮罩排除面板内部，允许阴影进入上下
圆角外侧。中段阴影的颜色、模糊、偏移和展开淡入参数保持不变。遮罩只处理
面板右侧的一条窄区域，避免整窗离屏纹理。

取证与前后图位于 `.artifacts/python/window-rim-diagnosis/`。本轮从收起状态的
真实 Gallery 内容生成模糊源，不再把已经展开的用户截图作为亚克力输入。
隐藏 D3D11 抓帧仍不等同于可见桌面的 DWM 合成，最终可见效果需独立确认。

下面的 `8a9a0188` 记录仅说明亚克力层的中间修正，不能覆盖独立阴影。

## 2026-09-20：亚克力子层修正（未完整解决用户反馈）
