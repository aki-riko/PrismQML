...
以下问题均已用真实隐藏运行链复现；P7E-P7G 已于 2026-07-13 完成，P7H 已于 2026-07-14 完成，P7I 正在按调用链拆批整改，其中 P7I-A/P7I-B/P7I-C 与 P7I-D1-D5 已完成，P7I-F 已进入 F1，失效 userCard API 前置清理、F1a 纯 QML 组合与 F1b Engine/context/provider 装配均已完成，下一批为 F1c root/pending/Splash 收尾拆分；P7J-P7K 仍按顺序待整改。所有 Python/QML 诊断均经统一 runner 执行并保持 `visible_windows=0 / job_active_processes=0`；D5 及更早批次已有用户桌面确认，用户随后也确认 F1a 门禁未出现错误弹窗，当前 F1b 仍只记录 runner 的零窗口/零残留证据。
