...
5. **P7I 剩余规范库存（进行中）**
   ...
   - F5 Research 同时发现 `system_tray.addMenu()` 写入的 `submenuActions` 全仓无消费者，QML 路径只创建带箭头父项而未连接真实 submenu；这不是可直接固化的机械合同，后续必须先用隐藏 QQmlEngine 真实复现并单独做正确性决策。下一批 F5c 优先进入 `WindowCore.show()` before characterization，只锁定新建/复用、双 restore、current 登记、lazy/eager 页面创建、重复 show 与异常部分状态，不混入 AUMID 长标识、SystemTray、Mica 或 App 生命周期。F6 处理其余 models/query tools，F7 处理 core/providers/state。每批仍须先补真实合同、再机械拆分，并重复库存、Python 3.9 AST、全量 Python、QML probe、headless CTest、changed scanner、用户配置/缓存隔离与 `git diff --check`；未使用导入与文件头继续后置。
