...
5. **P7I 剩余规范库存（进行中）**
   ...
   - F5 Research 同时发现 `system_tray.addMenu()` 写入的 `submenuActions` 全仓无消费者，QML 路径只创建带箭头父项而未连接真实 submenu；这不是可直接固化的机械合同，后续必须先用隐藏 QQmlEngine 真实复现并单独做正确性决策。F5b 继续只选一条剩余 window 调用链；F6 处理其余 models/query tools，F7 处理 core/providers/state。每批仍须先补真实合同、再机械拆分，并重复库存、Python 3.9 AST、全量 Python、QML probe、headless CTest、changed scanner、用户配置/缓存隔离与 `git diff --check`；未使用导入与文件头继续后置。
...
| P7 Python 规范债务 | 进行中 | 已完成框架签名/生成枚举例外定义、异常边界前四批、P7E 配置事务/schema/测试隔离、P7F DPI/WindowType 严格合同、P7G QRCode QML URL 传输协议、P7H NativeWindow 状态事务，以及 P7I-A ShadowManager、P7I-B WindowCore、P7I-C SqlListModel、P7I-D1 logger、P7I-D2 WindowCompat、P7I-D3 WindowBuilder、P7I-D4 SplashBuilder、P7I-D5 PageManager traceback 边界。P7I-F 的 F1 前置 userCard 清理、F1a/F1b/F1c、F2 `create_splash()`、F3a keyset 正确性/SQL 合同、F3b `_fetch_page()`、F4 PageManager 页面加载链与 F5a `WindowCore.addPage()` 机械拆分均已完成；统一 runner 已全局隔离 QML 磁盘缓存。F5a 最终门禁为聚焦 `10 passed`、联合 Python `83 passed`、两个真实 standalone 退出 0、隔离全量 Python `1123 passed / 1 skipped`、QML `169 OK / 0 错误 / 12 跳过`、C++ headless `8/8`、changed QML `0`、Python 3.9 AST 226 个输入、compileall 与 `git diff --check` 全绿；首次缓存干扰已精确归因外部 Kaleidos-android 两份 `.qmlc`，独占同一全量入口复验后配置、两类 QML 缓存与两份 audit 日志指纹完全一致，全部 runner 零可见窗口/零残留进程。`_create_window()` 已从 223 行降至 20 行，`create_splash()` 已从 207 行降至 30 行，`_fetch_page()` 已从 130 行降至 15 行，`_create_page()` 已从 93 行降至 15 行，`_start_async_page_load()` 已从 112 行降至 29 行，`addPage()` 已从 60 行降至 28 行，生产超长函数库存 `50→39`；下一批继续 F5b 剩余 window 单一调用链，SystemTray submenu 断链须先真实复现而不得机械固化。P7J-P7K 及剩余导入、文件头、benchmark 路径与测试 stdout 政策仍待处理 | `8272d2c0`、`354777fe`、`a8e80e86`、`c962014e`、`bb81d37b`、`e344c8a0`、`e48bb127`、`dafa2a34`、`1512f723`、`8e24b825`、`5fb9e7d8`、`620da260`、`77d045c5`、`cb19eb88`、`fe7a4f4e`、`21211777`、`3b717770`、`f2b4af11`、`cc4567b2`、`ef07928d`、`6db88a88`、`6bf8569f`、`0c482ed0`、`48171374`、`27e831f0`、`d20053c9`、`798f92c`、`42075fa5`、`d2f99369`、`32056b02`、`71854655`、`1b5e072`、`093b7dec` |
