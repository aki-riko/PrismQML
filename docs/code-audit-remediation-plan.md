...
| P7 Python 规范债务 | 进行中 | 已完成框架签名/生成枚举例外定义与首批异常边界：元数据缺包回退收窄，DWM 不再吞 process-control 异常，logger.exception 真实输出 traceback，同一 LogRecord 跨 handler 仅输出一次堆栈；Python 598/1、QML 169/0/12、CTest 6/6、Build All 七项全绿。剩余宽捕获、长函数、导入与文件头继续处理 | `8272d2c0`、`354777fe`、`a8e80e86` |
