...
P6D Chart 多系列提示框与标题组三提交已完成：`2c69c199` 将 `ChartMultiTooltip` 的样式注释降为普通局部说明，清理 1 条 QML009；`14901b38` 统一 `ChartTitle` 的公开属性与内容分节，并将布局说明降为普通局部注释，清理 2 条 QML009。两批非注释代码多重集合与父提交完全一致，不改提示框尺寸、颜色、阴影、显隐动画、格式化回调、标题/副标题内容、布局或公开 API。使用同一真实 QML 输入在修前、修后验证 Mon/A/B/Total 九项文本契约、自定义 `$` 格式化、提示框可见几何及 Sales/Week 标题，均零 warning、零窗口泄漏；Prism Design 皮肤与 Chart 联合回归 `11 passed`。最终完整门禁为 Python `2240 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、changed `3→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。全库总库存 `2,236→2,233`，QML008 1,342 保持不变、QML009 `851→848`，QML010 43 保持全部位于 examples。

P6D Chart 单值提示框与折线内容组三代码提交已完成：`d3036a97` 统一 `ChartTooltip` 的内容分节并将三角指针降为普通局部说明，清理 2 条 QML009；`0e17cf38` 将 `LineChartContent` 的内部属性、只读状态、信号、方法、根重绘 handler、Canvas 函数及 Timer 状态恢复到规范成员区，并统一公开属性、内部属性、只读状态、内部方法与内容分节，清理 20 条 QML008、7 条 QML009。两批均不改提示框尺寸/格式化/指针绘制、折线范围、坐标公式、Painter 调用、动画参数、marker、hover/click/wheel 信号或公开 API；非注释代码多重集合与父提交完全一致。ChartTooltip 修前修后验证 `CPU / 12.5%`、颜色点及 `10×6` 指针 Canvas，相关回归 `4 passed`；LineChartContent 修前修后验证单系列 3 点、多系列 `2×3` 点、动画 Canvas、hover/click 与数值 `30/10`，相关回归 `10 passed`，均零 warning、零窗口泄漏。首次完整门禁真实捕获未改动的 SegmentedControl 布局重定向测试反向采样 `237.46→196.42`、目标 `747`；该用例 fresh process `20/20`、QML 整套 `210/210` 后，100 场景同一 QGuiApplication 压力输入仍以 `2/100` 复现，证明不是可忽略的单次慢机时序。`51d761d9` 保持显式索引切换立即启动，只让 `items/x/width` 布局同步等待选中 delegate 的 `x/width` 连续两个事件轮次稳定；旧输入修后普通逐帧 `5/5`、压力 `200/200`，无反向回退且不延迟点击首帧。最终完整门禁为 Python `2240 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、changed 零新增与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。全库总库存 `2,233→2,204`，QML008 `1,342→1,322`、QML009 `848→839`，QML010 43 保持全部位于 examples。

P6D Chart 折线标记与饼图区域组三提交已完成：`bd41d143` 将 `LineChartMarkers` 的回调属性归入公开属性区、统一内容分节，并把 marker delegate 的四个派生属性前移到 anchors 之前，清理 4 条 QML008、3 条 QML009；`b787ffd5` 统一 `PieChartArea` 的公开属性与内容分节，将标题、图表区、tooltip 和 legend 降为普通局部说明，清理 5 条 QML009。两批非注释代码多重集合与父提交完全一致，不改最小/最大/平均值算法、marker 几何/Canvas、饼图数据、donut 中心强调、tooltip、legend、信号或公开 API。LineChartMarkers 修前修后验证最大值 30、最小值 10、平均值 20.00 及两个实际气泡，相关回归 `3 passed`；PieChartArea 修前修后验证 A/B 两项 donut、Distribution/Today 标题、中心强调值 60 与 `60 (60%)` tooltip，相关回归 `3 passed`，均零 warning、零窗口泄漏。最终完整门禁为 Python `2240 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、changed `12→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。全库总库存 `2,204→2,192`，QML008 `1,322→1,318`、QML009 `839→831`，QML010 43 保持全部位于 examples。

P6D Chart 饼图内容与雷达区域组三提交已完成：`f108018f` 将 `PieChartContent` 的根 hover/data handler 前移到内容之前、Canvas 状态前移到 anchors 之前、hover 重绘 handler 前移到 Timer 之前，并整理动画 Timer 状态与内容分节，清理 7 条 QML008、3 条 QML009；`128d823c` 统一 `RadarChartArea` 的公开属性、内部方法与内容分节，将标题、图表区、tooltip 和 legend 降为普通局部说明，清理 6 条 QML009。两批非注释代码多重集合与父提交完全一致，不改饼图角度/半径/标签/动画公式、hover/click/transition，或雷达图颜色、数据、tooltip、legend、信号及公开 API。PieChartContent 修前修后验证 A/B donut、动画完成、hover 0/1、click A 与 previousHoveredIndex 过渡状态，相关回归 `4 passed`；RadarChartArea 修前修后验证三个指标、S 系列、Radar/Today 标题及 `B: 40` tooltip，相关回归 `3 passed`，均零 warning、零窗口泄漏。最终完整门禁为 Python `2240 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、changed `16→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。全库总库存 `2,192→2,176`，QML008 `1,318→1,311`、QML009 `831→822`，QML010 43 保持全部位于 examples。

P6D Chart 雷达与散点内容组三提交已完成：`895e1c46` 将 `RadarChartContent` 的内部状态前移到信号之前、根重绘 handler 前移到内容之前、Canvas 状态前移到 anchors 之前，并整理动画 Timer 与内容分节，清理 8 条 QML008、5 条 QML009；`22e272c0` 对 `ScatterChartContent` 做同类成员恢复，清理 10 条 QML008、5 条 QML009。两批非注释代码多重集合与父提交完全一致，不改网格/坐标/绘制公式、颜色、动画参数、普通散点/effectScatter 分支、hit test、tooltip 数据、hover/click 信号或公开 API。RadarChartContent 修前修后验证三个实际 pointPositions、hover `0/1` 与 click 值 40，相关回归 `3 passed`；ScatterChartContent 修前修后验证两个普通点、一个 effectScatter 点及 `(2,3,isEffect=true)` 点击数据，相关回归 `8 passed`，均零 warning、零窗口泄漏。最终完整门禁为 Python `2240 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、changed `28→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。全库总库存 `2,176→2,148`，QML008 `1,311→1,293`、QML009 `822→812`，QML010 43 保持全部位于 examples。

P6D Chart XY 基类与数据缩放组三提交已完成：`fa3eb413` 统一 `XYChartCore` 的公开属性、只读状态、信号与内容分节，并把标题、图表区、网格及四组坐标轴说明降为普通局部注释，清理 8 条 QML008、10 条 QML009；`21485ebe` 将 `ChartDataZoom` 的完成 handler 前移到子元素之前，并把三组 `Connections` handler 前移到 `target` 赋值之前，清理 5 条 QML008。两批非注释代码多重集合与父提交完全一致，不改数值范围、零轴比例、散点范围、坐标几何、formatter、Canvas 缩略绘制、Slider 数值、最小视窗跨度、拖动 Timer、信号或公开 API。XYChartCore 修前修后使用同一真实输入验证 `valueRange=-14..34`、`zeroLineRatio=34/48`、散点 `x=0.8..3.2 / y=1.8..4.2`、标题/formatter 与 chartArea 几何，相关回归 `7 passed`；ChartDataZoom 修前修后验证缩略数据 `[10,-5,20]`、外部范围同步 `250..750`、最小跨度信号 `0.4..0.401` 与 `interactive=true→false`，隐藏基础窗口保持 2、可见窗口 0，相关回归 `7 passed`。最终门禁为 Python `2240 passed / 1 skipped / 1 warning`，该 warning 是 `test_headless_test_entrypoints` 子进程 reader thread 的既有 UTF-8 解码告警，退出码仍为 0，且不涉及本批 QML；QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 289、compileall、changed `23→0` 与 `git diff --check` 全部通过，所有 runner 零可见窗口、零残留。全库总库存 `2,148→2,125`，QML008 `1,293→1,280`、QML009 `812→802`，QML010 43 保持全部位于 examples。

P6D Chart 公开入口与 CircularGauge 组三提交已完成：`f9293b29` 将 `ChartView` 的尺寸/样式、tooltip、legend、饼图、雷达图、箱线图与空状态分组降为普通双语说明，并把平移 `MouseArea` 的三个按压状态 property 前移到自身赋值之前，清理 3 条 QML008、7 条 QML009；同批把视窗源码合同从违规注释锚点迁到真实 `ChartTooltip`/`ChartBottomLegend` 组件边界，避免规范化注释后测试失效。`bbed8c9c` 统一 `CircularGauge` 的公开方法术语，并把三个 Canvas 重绘生命周期 handler 前移到根尺寸与子元素之前，清理 3 条 QML008、1 条 QML009。两批生产 QML 的非注释代码多重集合与父提交完全一致，不改 Chart 数据切片、缩放/平移公式、tooltip/legend 数据、各图表 Loader、空状态、Gauge 进度/角度/Canvas 绘制、颜色、尺寸或公开 API。ChartView 修前修后相关真实回归 `11 passed`，覆盖视窗动画/缩放、Chart 强文本与 Prism Design；测试锚点修正前真实得到 `1 failed / 10 passed`，修正后恢复 `11 passed`。CircularGauge 修前修后同一 Prism Design 明暗主题输入均 `1 passed`，颜色与描边 token 保持一致；全部 runner 零可见窗口、零残留。最终门禁为 Python `2240 passed / 1 skipped / 1 warning`，重复出现的 warning 仍是 `test_headless_test_entrypoints` reader thread UTF-8 解码告警，退出码为 0，已保留为独立待办；QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 289、compileall、changed `14→0` 与 `git diff --check` 全部通过。全库总库存 `2,125→2,111`，QML008 `1,280→1,274`、QML009 `802→794`，QML010 43 保持全部位于 examples；除 P6E 的 `BarChartContent` 外，Chart 目录 P6D 当前已归零。

P6D DataWidget 与 PipsPager 分节组三提交已完成：`e45d3f77` 将 `DataWidgetCore` 的滚轮、阴影、卡片、表头、主体、内容、过渡、滚动助手、滚动条、空状态、骨架屏与底栏等 13 个局部视觉分组从非法标准分节降为普通双语说明；`4379301a` 将 `PipsPager` 的公开属性分节统一为 `Public Props 公开属性`，并把方向应用说明降为普通局部注释。两批非注释代码多重集合与父提交完全一致，不改 DataWidget 阴影销毁 fallback、滚轮/平滑滚动、横向 mixin、空态/加载态、布局/动画/公开 API，或 PipsPager orientation/vertical 绑定。DataWidgetCore 修前修后真实回归 `6 passed`，覆盖阴影销毁生命周期、聊天样式、Prism Design 明暗与综合皮肤；PipsPager 修前修后导航/轮播/综合皮肤回归 `3 passed`，均零可见窗口、零残留。最终门禁为 Python `2240 passed / 1 skipped / 1 warning`，重复 warning 仍为已记录的 `test_headless_test_entrypoints` reader thread UTF-8 解码告警；QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 289、compileall、changed `15→0` 与 `git diff --check` 全部通过。全库总库存 `2,111→2,096`，QML008 1,274 保持不变、QML009 `794→779`，QML010 43 保持全部位于 examples。

P6D PipsPagerCore 与 IndicatorBar 组三提交已完成：`ef1314ed` 将 `PipsPagerCore` 的公开/内部方法恢复到信号后、尺寸前，新增唯一标准内容分节，把滚轮、前后按钮与点容器降为普通局部说明，并将横竖点容器的 `_scrollOffset` property 前移到自身赋值之前，清理 8 条 QML008、5 条 QML009；`6a4701d3` 将 `IndicatorBar` 的两个颜色动画状态前移到内部属性区，统一尺寸分节并把颜色动画与渐变说明降为普通双语注释，清理 2 条 QML008、3 条 QML009。两批非注释代码多重集合与父提交完全一致，不改 PipsPager 页码边界、按钮可见性、横竖布局、滚动偏移、动画、信号/API，或 IndicatorBar 尺寸、方向、颜色、渐变、duration/easing。PipsPagerCore 修前修后同一真实输入验证 `cell=12`、方法 `3→4→3→7`、横竖偏移 `1/2/3` cell 与按钮可见性，联合回归 `3 passed`；IndicatorBar 修前修后逐帧验证垂直高度 `14→中间帧→36`、横向 `36×3` 与颜色动画收敛，Prism Design 明暗回归 `1 passed`，均零 warning、零可见窗口、零残留。最终门禁为 Python `2240 passed / 1 skipped / 1 warning`，重复 warning 仍为已记录的 `test_headless_test_entrypoints` reader thread UTF-8 解码告警；QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 289、compileall、changed `18→0` 与 `git diff --check` 全部通过。全库总库存 `2,096→2,078`，QML008 `1,274→1,264`、QML009 `779→771`，QML010 43 保持全部位于 examples。

P6D ImageWidget 与 Label 组三提交已完成：`374e3571` 统一 `ImageWidget` 的公开方法术语与内容分节，把占位符、圆角图片容器和交互降为普通局部说明，并将 `imageContainer` 的 layer/mask 赋值恢复到子 `Image` 之前，清理 4 条 QML008、4 条 QML009；`697670b9` 将 `Label` 的公开属性、内部属性、只读派生状态、信号和四个公开方法恢复到规范顺序，把样式绑定移到方法后、内容前，并统一唯一内容分节，清理 7 条 QML008、6 条 QML009。两批非注释代码多重集合与父提交完全一致，不改 ImageWidget 异步/缓存/HiDPI、图片状态、layer/mask 参数、fillMode/API，或 Label 字体/颜色/type 映射、wrap/elide、URL、hover/click 与公开方法。ImageWidget 修前修后同一真实 `1490×1490` PNG 输入均 Ready，layer 保持 `enabled/smooth/8`、fillMode `stretch→fit`，Prism Design 回归 `1 passed`；Label 修前修后验证正文 14px、wrap `true→false→true`、URL/get/clear、custom color 与 hyperlink 交互，相关联合回归 `7 passed`，均零 warning、零可见窗口、零残留。最终门禁为 Python `2240 passed / 1 skipped / 1 warning`，重复 warning 仍为已记录的 `test_headless_test_entrypoints` reader thread UTF-8 解码告警；QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 289、compileall、changed `21→0` 与 `git diff --check` 全部通过。全库总库存 `2,078→2,057`，QML008 `1,264→1,253`、QML009 `771→761`，QML010 43 保持全部位于 examples。

P6D ListView 与 ListWidget 三提交已完成：`b27cad86` 将 `ListView` 的公开/内部属性、信号、方法、尺寸与内容恢复到规范顺序，清理 7 条 QML008/QML009；同一真实 QML 输入验证 count `3→2`、外层 currentIndex `1→2`、内层回写 `→0`，轻量模式的 shadow/margin/header/frame 合同保持不变。机械整理 `ListWidget` 前，真实 `sortItems()` 输入先复现 `ListModel.get()` 活对象在 `clear()` 后失效，排序结果 `['', '', '']` 并产生 role undefined/type mismatch warning；进一步红测坐实排序后 current item 从 Beta 漂到 Alpha、多选仍绑定旧行号。`5f59f81b` 改为以文本快照生成顺序、通过 `ListModel.move()` 原地移动完整行，并按原始索引重映射 current/multi-selection，保留 icon/data/checkState/flags 等角色；新增公开 API 回归覆盖 add/insert/find/sort、setter、selection/current/take/clear，相关 ListWidget/Prism Design 联合 `4 passed`、零 QML warning。`39937af7` 再将 `ListWidget` 的成员与唯一标准分节恢复到规范顺序，清理 66 条 QML008、20 条 QML009；非注释代码多重集合与正确性提交完全一致。最终完整门禁为 Python `2241 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 284、compileall、changed `93→0` 与 `git diff --check` 全绿，所有 runner 零可见窗口、零残留。全库总库存 `2,057→1,964`，QML008 `1,253→1,181`、QML009 `761→740`，QML010 43 保持全部位于 examples。

P6D ListWidgetItem 与 TreeView 委托组三提交已完成：`1bf0134f` 将 `ListWidgetItem` 的按压、背景、光晕、选择条、内容行与鼠标区域从非法标准分节降为普通双语说明，并补唯一内容分节，清理 6 条 QML009；`1ebf18ba` 将 `TreeWidgetDelegate` 根派生属性、选择条 `_active` 与鼠标展开区域边界恢复到各自赋值之前，清理 7 条 QML008；`2f95b4be` 将 `TreeView` 的内部状态、公开/内部方法、轻量模式赋值、内部模型与默认委托恢复到标准顺序，并前移 delegate 与 MouseArea 派生属性，清理 10 条 QML008、5 条 QML009。三批非注释代码多重集合均与各自父提交完全一致，不改列表光晕几何/动画、树节点展开/折叠、hover/press、缩进、图标/文本或公开 API。ListWidgetItem 修前修后相关真实 QML `3 passed`；TreeWidgetDelegate 与 TreeView 修前修后同一 Prism Design TreeView/TreeWidget 输入均 `1 passed`，全部零 warning、零可见窗口、零残留。最终完整门禁为 Python `2241 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 284、compileall、changed `28→0` 与 `git diff --check` 全绿。全库总库存 `1,964→1,936`，QML008 `1,181→1,164`、QML009 `740→729`，QML010 43 保持全部位于 examples。

P6D TreeWidget 正确性、入口整理与 Marquee 三提交已完成：机械整理前，真实 QML 首先复现 `topLevelItem(0)` 传给 `setItemText/Icon/Data/CheckState()` 全部静默无效；排序后 current item 从 Beta 漂到 Alpha、多选仍绑定旧扁平索引；保存的 Beta `currentItem()` 快照在排序后调用 setter 会误改新路径上的 Alpha，重复文本也无法靠文本回退区分。`493a2714` 让原树节点、扁平模型与公开快照共享内部 `_treeItemId`，路径负责结构定位、稳定 ID 负责跨排序与重复文本身份解析，并在排序前保存真实对象引用、重建后恢复 current/multi-selection；新增回归覆盖顶级项四类 setter、重复文本第二项、排序选择重映射与持有快照跨排序修改，修后相关 TreeWidget/TreeView 联合 `2 passed`、零 QML warning。`dcc62aa2` 再将 `TreeWidget` 的公开/内部属性、信号、方法、尺寸、生命周期 handler、内部模型与内容恢复到标准顺序，前移 ListView 附加状态，清理 66 条 QML008、7 条 QML009；非注释代码多重集合与正确性父提交完全一致。`9777e1a8` 将 `Marquee` 的公开属性、只读状态、公开/内部方法、尺寸、生命周期 handler 与内容恢复到标准顺序，清理 10 条 QML008、1 条 QML009；非注释代码多重集合与父提交完全一致，明暗 Prism Design Marquee 输入修前修后均 `1 passed`。最终完整门禁为 Python `2242 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 285、compileall、changed `84→0` 与 `git diff --check` 全绿，所有 runner 零可见窗口、零残留。全库总库存 `1,936→1,852`，QML008 `1,164→1,088`、QML009 `729→721`，QML010 43 保持全部位于 examples。

P6D QRCode、Stepper 与 Watermark 分节组三提交已完成：`2ab6ed2c` 将 `QRCode` 的公开方法术语统一为 `Public Methods 公开方法`，并补标准尺寸与内容分节，清理 1 条 QML009；`edf32e43` 为 `Stepper` 补标准尺寸/内容分节，将背景线、进度线与步骤指示器降为普通双语说明，清理 3 条 QML009；`09dce8de` 为 `Watermark` 建立唯一标准内容分节，将内部常量、内容项与水印层降为普通双语说明，清理 3 条 QML009。三批非注释代码多重集合均与各自父提交完全一致，不改二维码请求/真实解码、Stepper 强调色/动画/点击、Watermark 几何/颜色/重复布局或公开 API。QRCode 修前修后真实 URL 协议、图像解码与 Prism Design 联合 `3 passed`；Stepper 强调色与明暗 Prism Design `3 passed`；Watermark 明暗输入 `1 passed`，均零 warning、零可见窗口、零残留。最终完整门禁为 Python `2242 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 285、compileall、changed `7→0` 与 `git diff --check` 全绿。全库总库存 `1,852→1,845`，QML008 1,088 保持不变、QML009 `721→714`，QML010 43 保持全部位于 examples。

P6D TableRowDelegate、PaintedRow 与 TableView 组三提交已完成：`94d82f8d` 将 `TableRowDelegate` 单元格控件容器的重挂载/居中 helper 前移到 anchors 与 visible 之前，清理 2 条 QML008；`329cc093` 统一 `PaintedRow` 的公开方法术语与尺寸/内容分节，将数据、几何、主题、回调和字体重绘 handler 前移到 Canvas 内容之前，清理 8 条 QML008、1 条 QML009，并将触及的说明统一为英文在前的双语注释；`801d9eff` 将 `TableView` 的公开/内部方法前移到 DataWidgetCore 覆盖项与生命周期 handler 之前，前移表头 delegate 的 hover 派生属性，并统一尺寸/内容分节，清理 5 条 QML008、5 条 QML009。三批非注释代码多重集合均与各自父提交完全一致，不改单元格控件重挂载/居中、Canvas 绘制/extraDraw/hover 命中、列宽算法、表头布局、滚动边距或公开 API。TableRowDelegate/PaintedRow 相关 TableWidget 与主题回归 `4 passed`；PaintedRow 入口 `4 passed`；TableView 明暗布局、主题入口与滚动边距 `5 passed`，均零 warning、零可见窗口、零残留。最终完整门禁为 Python `2242 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 285、compileall、changed `21→0` 与 `git diff --check` 全绿。全库总库存 `1,845→1,824`，QML008 `1,088→1,073`、QML009 `714→708`，QML010 43 保持全部位于 examples。
...
P6D navigation 资料卡、分页器与菜单栏组三提交已完成：`8c6a0bf9` 将 `NavigationProfileCard` 的背景、头像、文本区与交互分组收敛到唯一标准内容分节，清理 4 条 QML009；`a11db155` 将 `Paginator` 的内部 `_targetX` 前移到尺寸赋值之前，补标准内容分节并把上页、页码视口和下页说明降为普通双语注释，清理 1 条 QML008、3 条 QML009；`02d7c64d` 将 `MenuBar` delegate 状态与 `_openMenuAt()` 前移到子元素之前，恢复 `Connections` 函数及 `Menu` handler 的规范顺序并统一内容分节，清理 5 条 QML008、1 条 QML009。三批非注释代码多重集合均与各自父提交完全一致；修前修后真实 QML 回归分别为 `2 passed`、`1 passed`、`6 passed`。Paginator 提交前检查曾捕获一处新空白行尾空格，已在未推送阶段修正并 amend，最终提交 `git show --check` 无输出。最终完整门禁为 Python `2244 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 287、compileall、changed `14→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 820 条；全库总库存 `1,769→1,755`，QML008 `1,044→1,038`、QML009 `682→674`，QML010 43 保持全部位于 examples。
...
P6D chat 收口两提交已完成：`5a82d62c` 将 `MarkdownView` 的派生块状态、解析方法、尺寸与内容恢复到规范顺序，并前移 delegate 的 required 属性，清理 3 条 QML008、2 条 QML009；`8a2a135e` 将 `ChatBubble` 的内部展开状态、全部只读几何、尺寸、内容变化 handler 与子元素恢复到规范顺序，清理 8 条 QML008、7 条 QML009。两批同步把触达注释统一为英文在前的双语格式，非注释代码多重集合分别保持 `86=86` 与 `190=190`；修前基线到修后 changed `20→0`，chat 联合真实 QML 回归均为 `4 passed`，零 warning、零可见窗口、零残留。至此 `controls/chat` QML008/QML009 完全归零。最终完整门禁为 Python `2244 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 287、compileall、changed 0 与 `git diff --check` 全绿。P6D 累计清理 1024 条；全库总库存 `1,571→1,551`，QML008 `909→898`、QML009 `619→610`，QML010 43 保持全部位于 examples。
...
P6D inputs Toggle 指示器组三提交已完成：`dd4566c2` 将 `ToggleSwitchIndicator` 的公开属性、只读状态、轨道样式、内容与交互说明恢复为规范分节，清理 4 条 QML009；`f887deb0` 将 `ToggleRadioIndicator` 的全部只读颜色状态前移到尺寸之前，并统一公开属性、尺寸、视觉样式与内容分节，清理 2 条 QML008、3 条 QML009。两批非注释代码多重集合分别保持 `92=92` 与 `63=63`；Toggle 真实回归均为 `5 passed`，两次全组件 probe 均为 `169/0/12`，全部 runner 零可见窗口、零残留。最终完整门禁为 Python `2244 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 287、compileall、changed `9→0` 与 `git diff --check` 全绿。P6D 累计清理 1045 条；全库总库存 `1,539→1,530`，QML008 `898→896`、QML009 `598→591`，QML010 43 保持全部位于 examples。

P6D inputs ColorPicker 双滑块组三提交已完成：`cada4e32` 将 `ColorPickerBrightnessSlider` 的公开属性、内容/手柄/交互说明恢复为规范术语，并把 MouseArea 更新函数前移到自身赋值之前，清理 1 条 QML008、4 条 QML009；`43a01a01` 对 `ColorPickerHueSlider` 做同构整理并补齐文件头触达说明的双语格式，同样清理 1 条 QML008、4 条 QML009。两批非注释代码多重集合分别保持 `65=65` 与 `62=62`，clamp、渐变、拖动算法和全部数值逐字不变；picker 相关真实回归均为 `5 passed`，两次全组件 probe 均为 `169/0/12`，全部 runner 零可见窗口、零残留。最终完整门禁为 Python `2244 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 287、compileall、changed `10→0` 与 `git diff --check` 全绿。P6D 累计清理 1055 条；全库总库存 `1,530→1,520`，QML008 `896→894`、QML009 `591→583`，QML010 43 保持全部位于 examples。

P6D inputs ColorCircles token 与 ColorPalette 组三提交已完成：`a1c9c9a` 先经公共 `ColorPicker(type_circle)` 建立真实 QML 几何基线，锁定默认圆点 `28px`、选择容器 `36px`、外环 `34px / opacity=0.6`、零 warning 和零新增窗口；`443955e9` 将 `ColorCircles` 的默认尺寸、容器/外环增量、边框与透明度迁到既有 spacing/border/opacity token，仅新增缺失的 `colorPickerMetrics.circleHoverOpacity=0.8`，同时前移 delegate 状态并清理 2 条 QML008、1 条 QML009。首次实现误用了不存在的 `Enums.opacity.secondary`，同一基线立即红测为外环 `opacity=1.0`；源码核验 `Enums.qml` 唯一公开 alias 为 `opacityLevel` 后修正，同一真实输入恢复 `0.6`，未提交错误版本。`24b5250f` 再将 `ColorPalette` 三个尺寸属性前移到 signals 之前并统一公开属性分节，清理 3 条 QML008、1 条 QML009，非注释代码多重集合保持 `176=176`。最终完整门禁为 Python `2246 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 288、compileall、changed `7→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1062 条；全库总库存 `1,520→1,513`，QML008 `894→889`、QML009 `583→581`，QML010 43 保持全部位于 examples；ColorCircles token 债务至此关闭。

P6D inputs ColorPickerChannelSlider token 批已完成：`ae32a02` 先直接加载内部组件，锁定 `260×24` 尺寸、handle 从 `0` 动画到轨道终点，以及 R/G/B/A 四通道渐变两端颜色；测试首次立即读取 handle 时停在动画起点，核验 `Enums.duration.fast=100ms` 后按 150ms 事件循环余量读取，同一输入稳定通过。`5936b517` 将通道默认值、输入下界、四个 switch case、渐变位置、棋盘奇偶与拖动 clamp 迁到既有通道/opacity token，新增缺失的 `channelMinValue=0` 与 `checkerboardParity=2`，并把 MouseArea 更新函数前移到自身赋值之前，清理 1 条 QML008、2 条 QML009。四通道/handle 基线、token 源码合同与 picker 联合回归 `8 passed`，全组件 probe `169/0/12`。最终完整门禁为 Python `2249 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 AST 289、compileall、changed `3→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1065 条；全库总库存 `1,513→1,510`，QML008 `889→888`、QML009 `581→579`，QML010 43 保持全部位于 examples。

P6D inputs ComboBox 内部组件组三提交已完成：`1fdb2ad` 先用隐藏真实 QML 场景锁定 `PopupSearchBox` 的文本信号、`clear()`、启停尺寸与 `focusInput()`，以及 `ComboBoxPopupContent` 的 `180×90` 几何、滚动阈值和 ListView `parentControl` 透传，两个场景均零 warning、零可见窗口；`3bb86041` 将 `PopupSearchBox` 的公开方法前移到尺寸赋值之前，并统一公开属性与内容分节，清理 2 条 QML008、2 条 QML009；`af5c039c` 将 `ComboBoxPopupContent` 的只读滚动状态前移到尺寸之前、ListView 的 `parentControl` 前移到自身赋值之前，并统一公开属性、只读状态、尺寸与内容分节，清理 3 条 QML008、1 条 QML009。两个产品文件的非注释代码多重集合分别保持 `29=29` 与 `38=38`，同一基线扩展为源码归零护栏后专项 `4 passed`。最终完整门禁为 Python `2253 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 grammar AST 290、compileall、changed `8→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1073 条；全库总库存 `1,510→1,502`，QML008 `888→883`、QML009 `579→576`，QML010 43 保持全部位于 examples。

P6D inputs ComboBoxStyleHelper token 批已完成：`020ca32` 先用隐藏真实 QML 场景逐态锁定 default/primary/transparent 三种样式的默认、hover、pressed、popup、disabled 背景色，以及空文本、主样式、禁用文本色和边框色，修前基线 `6 passed`、零 warning、零可见窗口；`4724a8e8` 将 `style===1/2` 迁到既有 `Enums.comboBox.style_primary/style_transparent`，把 primary popup/pressed/hover 的 `1.1/1.15/1.08` 提取为 `Enums.comboBox` 公开颜色系数，并把三个自创分节收敛为唯一 `Internal Methods 内部方法` 与普通双语说明，清理 3 条 QML009。token 值、三种样式逐态颜色和源码引用护栏扩展后聚焦回归 `13 passed`。最终完整门禁为 Python `2257 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 grammar AST 290、compileall、changed `3→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1076 条；全库总库存 `1,502→1,499`，QML008 883 保持不变、QML009 `576→573`，QML010 43 保持全部位于 examples。

P6D inputs ComboBoxFont token 批已完成：`60d9107` 先用隐藏真实 QML 场景锁定十项默认字体列表、`Segoe UI` 默认选择、`model` 绑定、自定义列表/选择覆盖、`220px` 宽度与 popup item 高度，修前基线 `9 passed`、零 warning、零可见窗口；Research 另用同一 headless runner 验证当前隔离桌面中的 `Qt.fontFamilies()` 返回空列表，因此没有把真实字体枚举误当作稳定默认源。`8c9b2403` 将固定字体列表、默认字体与 delegate 的 `100px` fallback width 等值迁到 `Enums.comboBox`，组件通过 `slice()` 获取独立数组，并统一公开属性、信号、内容分节和 delegate 属性顺序，清理 1 条 QML008、2 条 QML009。默认值、覆盖行为、源码 token 与归零护栏扩展后聚焦回归 `14 passed`。最终完整门禁为 Python `2260 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 grammar AST 290、compileall、changed `3→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1079 条；全库总库存 `1,499→1,496`，QML008 `883→882`、QML009 `573→571`，QML010 43 保持全部位于 examples。

P6D inputs SpinBox token 批已完成：`09e19a1` 先用公共 `SpinBox` 隐藏真实 QML 场景锁定 normal/double/compact/compact-double 四种类型的 decimals、stepSize、compactMode、implicitWidth/Height 与 displayValue，并保留 normal/compact 按钮随宿主高度变化的既有回归，修前基线 `4 passed`、零 warning、零可见窗口；`4d782bb0` 将整数/小数精度与步长迁到 `Enums.input`，复用现有 `Enums.controlSize.inputHeightCompact`，仅新增缺失的 compact-double 额外宽度 token，并把四个自创分节统一为公开属性、只读状态、普通配置说明与尺寸分节，清理 4 条 QML009。四类型运行时、按钮父链、主题入口与源码 token 护栏聚焦回归 `13 passed`。最终完整门禁为 Python `2263 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 grammar AST 290、compileall、changed `4→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1083 条；全库总库存 `1,496→1,492`，QML008 882 保持不变、QML009 `571→567`，QML010 43 保持全部位于 examples。

P6D inputs LineEditLabel token 批已完成：`919b395` 先经公共 `LineEdit(inputType=type_label)` 的隐藏真实父链锁定初始居中标签、有内容后 `y=spacing.s` 与 caption/body 缩放、`clear()` 回落、文本信号、TextInput 高度/字体和 enabled 透传，修前基线 `3 passed`、零 warning、零可见窗口；测试首次直接读取 `textInput` alias 时 PySide 无法转换 `QQuickTextInput*`，按实际源码属性遍历定位真实 TextInput 后同一输入通过，未改产品。`23e25732` 将 `24px` 文本框高度与 `1.0` 静止缩放迁到 `Enums`，把五个自创分节统一为公开属性、只读状态、公开方法、唯一内容分节和普通双语说明，清理 5 条 QML009。浮动/回落动画终态、字体、生命周期、主题入口与源码 token 护栏聚焦回归 `12 passed`。最终完整门禁为 Python `2268 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 grammar AST 291、compileall、changed `5→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1088 条；全库总库存 `1,492→1,487`，QML008 882 保持不变、QML009 `567→562`，QML010 43 保持全部位于 examples。

P6D inputs CalendarPicker token 批已完成：`6c4f2cd` 先用 single/range 两个公共 `CalendarPicker` 隐藏真实实例锁定固定日期/范围格式、reset 占位、周标题、月份格式、单选/范围宽度、图标透明度和关闭态 popup 配置，修前基线 `2 passed`、零 warning、零可见窗口；测试曾把 `calPopup` QQuickItem 的普通 `visible=true` 误当作 native 窗口可见，按 runner 的真实顶层窗口差集修正判据后同一输入通过。`bbf09cc6` 将日期字段边界、补零/分隔符、无选择日、单选/范围宽度、图标透明度与 slide-down 动画类型迁到 `Enums`，并把 `displayDate` 前移到只读状态、`_formatDate` 归入内部方法、删除重复 Public Methods 分节，将交互和弹层降为内容区普通说明，清理 1 条 QML008、5 条 QML009。单选/范围、CalendarPickerCore 叶组件、Prism Design 输入与源码 token 护栏聚焦回归 `13 passed`。最终完整门禁为 Python `2272 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 grammar AST 292、compileall、changed `6→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1094 条；全库总库存 `1,487→1,481`，QML008 `882→881`、QML009 `562→557`，QML010 43 保持全部位于 examples。

P6D inputs PinInput token 批已完成：`2fefd58` 先用 default/custom 两个公共 `PinInput` 隐藏实例锁定默认长度、自定义四格几何、隐藏 TextInput、valueModified/completed、密码掩码、明文切换和 clear 生命周期，修前基线 `2 passed`、零 warning、零可见窗口。Repeater delegate 不在普通 QObject ownership 树中，最终通过 Qt Quick `childItems()` 视觉树与 children 去重遍历定位；中间一次仅用于诊断的未知属性批量读取进程以 `0xC0000409` 退出，runner 确认零窗口、零残留，未继续该路径。`9981cdc5` 将默认长度、normal echo mode、掩码字符与 cursor/隐藏输入 opacity 迁到 `Enums`，将 focused 前移为只读状态、三个 delegate 状态前移到尺寸之前，并把 Hidden Input 分节降为内容区普通说明，清理 4 条 QML008、2 条 QML009。PIN 生命周期、Prism Design、主题入口与源码 token 护栏聚焦回归 `8 passed`。最终完整门禁为 Python `2276 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 grammar AST 293、compileall、changed `6→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1100 条；全库总库存 `1,481→1,475`，QML008 `881→877`、QML009 `557→555`，QML010 43 保持全部位于 examples。

P6D inputs ColorPickerInputs 批已完成：`cfbef17` 先直接创建内部组件，锁定 `#336699` 的 `_r/_g/_b=51/102/153`、HEX 文本与三条 slider 值，并用真实 slider `valueModified(255)` 和 Hex `editingFinished(ABCDEF)` 验证双向更新、两次 colorChanged、零 warning/零可见窗口，修前基线 `2 passed`。Hex 定位最初同时匹配三个 ChannelSlider TextInput，加入源码已确认的 `maximumLength=6` 后唯一定位。`4e173d6c` 将四个内部派生属性前移到 signals 之前、`onSelectedColorChanged` 前移到子元素之前，统一公开/内部属性分节，并新增 mode cycle step token，清理 5 条 QML008、2 条 QML009。Research 曾同步替换 `ColorPickerDropdown` 的同值步长，但其 touched-file 存量为 8 条，changed scanner 显示 `current=8`；因缺少该组件修前父链基线而完整恢复，留待独立批次，最终本批 changed `7→0`。聚焦回归 `11 passed`。最终完整门禁为 Python `2280 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 grammar AST 294、compileall、changed `7→0` 与 `git diff --check` 全绿，全部 runner 零可见窗口、零残留。P6D 累计清理 1107 条；全库总库存 `1,475→1,468`，QML008 `877→872`、QML009 `555→553`，QML010 43 保持全部位于 examples。

P6D inputs ColorPickerPanel 批已完成：`ffd90b9` 用真实 QML 窗口锁定默认 `260×200`、HSV 默认值、selector 中心与越界 clamp、黑白对比边框，并通过两次真实鼠标点击验证 hue/saturation 映射与 colorChanged 次数；同时连接 Canvas `painted`，对亮度 `1.0→0.25` 前后真实渲染像素 `128/255/255→32/64/64` 取样，修前基线 `3 passed`、零 warning，runner 零可见窗口/零残留。`1e7ddc1a` 将属性、信号、尺寸、handler、子元素与 MouseArea 方法恢复规定顺序，把五个自创分节收敛为标准公开属性/信号/尺寸/内容分节，并复用 HSV、面板宽度、opacity 与动画 token，仅补充面板默认高度、Canvas 列宽和 selector 亮度判定 token，清理 2 条 QML008、5 条 QML009。ColorPicker 联合回归 `17 passed`。最终完整门禁为 Python `2285 passed / 1 skipped`、QML probe `169 OK / 0 错误 / 12 跳过`、headless CTest `9/9`、Windows native CTest `2/2`、MkDocs strict、Python 3.9 grammar AST 295、compileall、changed `7→0` 与 `git diff --check` 全绿，全部 runner 零弹窗、零可见窗口、零残留。P6D 累计清理 1114 条；全库总库存 `1,468→1,461`，QML008 `872→870`、QML009 `553→548`，QML010 43 保持全部位于 examples。
...
截至 2026-07-17，按剩余工作量粗估整体计划约完成 **91%**：P0–P5、P7J、P7K 与 P8 已完成，P6/P7 继续进行，P9 待执行；该百分比不是按阶段数量简单平均，新增插单会改变剩余时间估算。
...
| P6 QML 规范债务 | 进行中 | P6A–P6C21 与 P6D 已完成的小批记录保持不变；扫描器已覆盖字符串、固定数值、数组、callable、examples QML010 与高置信 primitive 传播，P6C16–21 已清理全部生产颜色且未新增生命周期例外，P6D 累计清理 791 条成员顺序/分节术语债务，`controls/feedback` 与 `controls/data`（除 P6E 的 BarChartContent）当前 QML008/QML009 已归零并进入 `navigation`；DialogBoxCore 快速重复提交、ListWidget/TreeWidget/TableWidget 行身份失效均以真实 QML 红测闭环。当前全库为 1,784 项：QML008 1,051、QML009 690、QML010 43、QML011 0、QML013 0；43 条 QML010 全部属于 examples，生产颜色债务归零。最新全量 Python 2243/1、QML 169/0/12、CTest 9/9 + native 2/2、MkDocs strict、Python 3.9 AST 286、compileall、changed 0 | `d5b5852`、`8e3ba4b0`、`e98adebb`、`557930af`、`b0d23808`、`49d6d6d0`、`09c696df`、`a877c2c7`、`6fc6e645`、`2a22c115`、`06eb4af9`、`685d063e`、`bcf0c737`、`4c22e7bc`、`bad57911`、`b281b7b`、`85a75ff3`、`0e438e60`、`1f9d1038`、`b1050f84`、`d4e2e11e`、`10b25427`、`8c633f4f`、`6810e12c`、`1758f484`、`49a899d5`、`0946eea5`、`2c1be8dd`、`cbf934d2`、`4e383715`、`bff2b32b`、`95c3db84`、`41565d4b`、`e17b06ca`、`c284433f`、`744eb6ec`、`ca6eb64b`、`af0b0cd4`、`28664920`、`a17f46a8`、`263851c1`、`165dbd4`、`8e29044c`、`86d7e0ee`、`dc5cf90`、`f329a3e`、`668cfda`、`89474354`、`78ff168`、`4e2e4401`、`369724f`、`8d1860f8`、`2f3c7d3`、`dbe71d46`、`f51f302`、`eb044ed8`、`c5bf1f0`、`54879042`、`4597651a`、`5694624d`、`e6819b23`、`14f4eea3`、`c7b07546`、`eb455b80`、`ed580d34`、`1e9d6aa8`、`f339dd29`、`074c7070`、`691a2c8c`、`9cf8fe69`、`d77d9a50`、`133af05c`、`5a576ef9`、`0811cc78`、`cc8a51c1`、`511e14ba`、`afd11daf`、`f6b1891f`、`d16dab7d`、`440773a5`、`a9ddba12`、`f3308d32`、`daa37c2e`、`112f1afa`、`ad5e8a3e`、`fd6062f3`、`702835a3`、`e2231281`、`dbd71743`、`57d94b48`、`ff650be9`、`37972d3a`、`6fc7eb2a`、`2c69c199`、`14901b38`、`d3036a97`、`0e17cf38`、`51d761d9`、`bd41d143`、`b787ffd5`、`f108018f`、`128d823c`、`895e1c46`、`22e272c0`、`fa3eb413`、`21485ebe`、`f9293b29`、`bbed8c9c`、`e45d3f77`、`4379301a`、`ef1314ed`、`6a4701d3`、`374e3571`、`697670b9`、`b27cad86`、`5f59f81b`、`39937af7`、`1bf0134f`、`1ebf18ba`、`2f95b4be`、`493a2714`、`dcc62aa2`、`9777e1a8`、`2ab6ed2c`、`edf32e43`、`09dce8de`、`94d82f8d`、`329cc093`、`801d9eff`、`c4525283`、`73a95a9e`、`b3145d4b` |
...
| P6D inputs（2026-07-17） | 进行中 | 前十五组二十三组件/测试批清理 90 条（QML008 28 / QML009 62），并关闭 ColorCircles、ChannelSlider、ComboBoxStyleHelper、ComboBoxFont、SpinBox、LineEditLabel、CalendarPicker、PinInput、ColorPickerInputs 与 ColorPickerPanel token/顺序债务；P6D 累计清理 1114 条。全库库存 1,461（QML008 870 / QML009 548 / QML010 43），最新完整门禁 Python 2285/1、QML 169/0/12、CTest 9/9 + native 2/2、MkDocs strict、Python 3.9 grammar AST 295、compileall、changed 0；ColorPicker circle 空 item 分支与 Toggle 三态枚举化仍为独立后续批 | `504eafaf`、`1b30d4f1`、`aadb6334`、`bfcdd484`、`082808ad`、`dd4566c2`、`f887deb0`、`cada4e32`、`43a01a01`、`a1c9c9a`、`443955e9`、`24b5250f`、`ae32a02`、`5936b517`、`1fdb2ad`、`3bb86041`、`af5c039c`、`020ca32`、`4724a8e8`、`60d9107`、`8c9b2403`、`09e19a1`、`4d782bb0`、`919b395`、`23e25732`、`6c4f2cd`、`bbf09cc6`、`2fefd58`、`9981cdc5`、`cfbef17`、`4e173d6c`、`ffd90b9`、`1e7ddc1a` |
| P9 最终验收 | 待执行 |  |  |
