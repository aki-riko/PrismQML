...
以下问题均已用真实隐藏运行链复现；P7E-P7G 已于 2026-07-13 完成，P7H 已于 2026-07-14 完成，P7I 正在按调用链拆批整改，其中 P7I-A/P7I-B/P7I-C 与 P7I-D1-D5 已完成，P7I-F 的 F1 三个子批已全部完成，下一批为 F2 `create_splash()` 调用链；P7J-P7K 仍按顺序待整改。所有 Python/QML 诊断均经统一 runner 执行并保持 `visible_windows=0 / job_active_processes=0`；D5 及更早批次已有用户桌面确认，用户随后也确认 F1a 门禁未出现错误弹窗，F1b/F1c 仍只记录 runner 的零窗口/零残留证据。
...
5. **P7I 剩余规范库存（进行中）**
   ...
   - F5 Research 同时发现 `system_tray.addMenu()` 写入的 `submenuActions` 全仓无消费者，QML 路径只创建带箭头父项而未连接真实 submenu；这不是可直接固化的机械合同，后续必须先用隐藏 QQmlEngine 真实复现并单独做正确性决策。下一批 F5d 严格进入 `WindowCore.__init__()` 状态初始化链的 before characterization；`_setAppIcon()` 与 P7K 路径编解码正确性耦合，SystemTray submenu、Mica、AUMID 长标识和 `App.__init__` 生命周期均继续后置，不混入机械拆分。F6 处理其余 models/query tools，F7 处理 core/providers/state。每批仍须先补真实合同、再机械拆分，并重复库存、Python 3.9 AST、全量 Python、QML probe、headless CTest、changed scanner、用户配置/缓存隔离与 `git diff --check`；未使用导入与文件头继续后置。
...
| P7 Python 规范债务 | 进行中 | 已完成框架签名/生成枚举例外定义、异常边界前四批、P7E 配置事务/schema/测试隔离、P7F DPI/WindowType 严格合同、P7G QRCode QML URL 传输协议、P7H NativeWindow 状态事务，以及 P7I-A ShadowManager、P7I-B WindowCore、P7I-C SqlListModel、P7I-D1 logger、P7I-D2 WindowCompat、P7I-D3 WindowBuilder、P7I-D4 SplashBuilder、P7I-D5 PageManager traceback 边界。P7I-F 的 F1 前置 userCard 清理、F1a/F1b/F1c、F2 `create_splash()`、F3a keyset 正确性/SQL 合同、F3b `_fetch_page()`、F4 PageManager 页面加载链、F5a `WindowCore.addPage()`、F5b AUMID 派生与 F5c `WindowCore.show()` 机械拆分均已完成；统一 runner 已全局隔离 QML 磁盘缓存。F5c 最终门禁为行为+结构 `44 passed`、窗口联合 `165 passed`、真实 standalone `7 passed / 10 deselected`、NativeWindow 启动 `14 passed`、全量 Python `1253 passed / 1 skipped`、QML `169 OK / 0 错误 / 12 跳过`、C++ headless `8/8`、changed QML `0`、Python 3.9 AST 238 个输入、compileall、Rust fmt/clippy/test `6/6` 与 `git diff --check` 全绿；配置与 audit 指纹保持基线，缓存最新 mtime 早于本批首个提交，全部 runner 零可见窗口/零残留进程。`_create_window()` 已从 223 行降至 20 行，`create_splash()` 已从 207 行降至 30 行，`_fetch_page()` 已从 130 行降至 15 行，`_create_page()` 已从 93 行降至 15 行，`_start_async_page_load()` 已从 112 行降至 29 行，`addPage()` 已从 60 行降至 28 行，`_derive_app_user_model_id()` 已从 35 行降至 29 行，`show()` 已从 45 行降至 7 行，生产超长函数库存 `50→37`；下一批 F5d 进入 `WindowCore.__init__()` 状态初始化链。超长 packaged executable 身份退化、`_setAppIcon()` 路径正确性与 SystemTray submenu 断链均须独立真实修复，不得机械固化或混入 F5d。P7J-P7K 及剩余导入、文件头、benchmark 路径与测试 stdout 政策仍待处理 | `8272d2c0`、`354777fe`、`a8e80e86`、`c962014e`、`bb81d37b`、`e344c8a0`、`e48bb127`、`dafa2a34`、`1512f723`、`8e24b825`、`5fb9e7d8`、`620da260`、`77d045c5`、`cb19eb88`、`fe7a4f4e`、`21211777`、`3b717770`、`f2b4af11`、`cc4567b2`、`ef07928d`、`6db88a88`、`6bf8569f`、`0c482ed0`、`48171374`、`27e831f0`、`d20053c9`、`798f92c`、`42075fa5`、`d2f99369`、`32056b02`、`71854655`、`1b5e072`、`093b7dec`、`a90beb8`、`2be95663`、`977966f`、`2d0881dc` |
