...
以下问题均已用真实隐藏运行链复现；P7E-P7G 已于 2026-07-13 完成，P7H 已于 2026-07-14 完成，P7I 正在按调用链拆批整改，其中 P7I-A/P7I-B/P7I-C 与 P7I-D1-D5 已完成，P7I-F 已进入 F1，失效 userCard API 前置清理已完成，下一批为 F1a `_create_window` 纯 QML 组合抽离；P7J-P7K 仍按顺序待整改。所有 Python/QML 诊断均经统一 runner 执行并保持 `visible_windows=0 / job_active_processes=0`；D5 及更早批次已有用户桌面确认，当前 F1 前置清理仅记录 runner 的零窗口/零残留证据。
...
| P7 Python 规范债务 | 进行中 | 已完成框架签名/生成枚举例外定义、异常边界前四批、P7E 配置事务/schema/测试隔离、P7F DPI/WindowType 严格合同、P7G QRCode QML URL 传输协议、P7H NativeWindow 状态事务，以及 P7I-A ShadowManager、P7I-B WindowCore、P7I-C SqlListModel、P7I-D1 logger、P7I-D2 WindowCompat、P7I-D3 WindowBuilder、P7I-D4 SplashBuilder、P7I-D5 PageManager traceback 边界。P7I-F 已进入 F1，失效 userCard API 前置清理门禁为删除 API `5 passed`、真实 BAR/SPLIT/FILLED 退出 0、headless 入口 `22 passed`、Python `887 passed / 1 skipped`、changed QML `0`、QML `169 OK / 0 错误 / 12 跳过`、C++ headless `8/8`，全部 runner 零可见窗口/零残留进程，用户配置/缓存/audit 指纹均未变化；本批未复用较早批次的用户桌面确认；`_create_window()` 仍为 223 行、超长函数库存仍为 50。下一批为 F1a 纯 QML 组合抽离；P7J-P7K 及剩余导入、文件头、benchmark 路径与测试 stdout 政策仍待处理 | `8272d2c0`、`354777fe`、`a8e80e86`、`c962014e`、`bb81d37b`、`e344c8a0`、`e48bb127`、`dafa2a34`、`1512f723`、`8e24b825`、`5fb9e7d8`、`620da260`、`77d045c5`、`cb19eb88`、`fe7a4f4e`、`21211777`、`3b717770`、`f2b4af11`、`cc4567b2`、`ef07928d`、`6db88a88` |
