...
以下问题均已用真实隐藏运行链复现；P7E-P7G 已于 2026-07-13 完成，P7H 已于 2026-07-14 完成，P7I 正在按调用链拆批整改，其中 P7I-A/P7I-B/P7I-C 与 P7I-D1-D5 已完成，P7I-F 已进入 F1，失效 userCard API 前置清理与 F1a 纯 QML 组合抽离已完成，下一批为 F1b Engine/context 注入拆分；P7J-P7K 仍按顺序待整改。所有 Python/QML 诊断均经统一 runner 执行并保持 `visible_windows=0 / job_active_processes=0`；D5 及更早批次已有用户桌面确认，当前 F1 批次仅记录 runner 的零窗口/零残留证据。
...
5. **P7I 剩余规范库存（进行中）**
   ...
   - F1a 最终门禁：纯 helper / window-builder 聚焦 `6 passed`，联合删除 API、转义与 WindowType 聚焦 `17 passed`；真实 BAR/SPLIT/FILLED composition、文件 component 后诊断失败和缓存目录占用 fallback 均退出 0，headless 入口矩阵 `22 passed`。最终 Python 全量 `891 passed / 1 skipped`，QML probe `169 OK / 0 错误 / 12 跳过`，headless CTest `8/8`，changed QML `0`，186 个受跟踪 Python 输入的 3.9 AST、compileall 与 `git diff --check` 全部通过。全部 Python/QML runner 为 `visible_windows=0 / job_active_processes=0`；真实用户缓存仍为 248 个文件且聚合 SHA-256 为 `EEB0DBFF690C88C938BC470EED4B22FB6D59A253F3CF78D4A8378F6A8D4D0519`，`app.json` 与两份受保护 audit 指纹均未变化。本批未复用较早用户桌面确认；代码与测试最终 Review 均为 P0/P1/P2/P3 全零。下一批严格进入 F1b。
   - F2 单独拆 `create_splash()` 207 行调用链，保留文件/inline component、挂载与非致命失败语义，目标主函数及 helper 均不超过 30 行；难度 4–8 小时、风险高。
   - F3 单独拆 `SqlListModel._fetch_page()` 129 行调用链，不混入 SQL 词法器或 P7K 只读连接：按 route → plan → backend dispatch → column install → format → return 分层，并让 `_resolve_columns()` 复用列安装 helper；目标 `_fetch_page ≤30` 行、深度不超过 2。必须补单库 Rust/Python fallback、ASC/DESC/混合/NULL/重复键与双 shard 连续翻页无重无漏合同，并用改前 worktree 做 median/p95 配对；难度 4–8 小时、风险高。
   - F4 单独拆页面同步/异步加载链：`_create_page()` 93 行、`_start_async_page_load()` 112 行及内部 `on_page_ready()` 67 行，保留 16ms 时序、尺寸信号、deferred 批创建与切换合同；难度 4–8 小时、风险高。
   - F5 再按单一调用链消化剩余 window 函数，F6 处理其余 models/query tools，F7 处理 core/providers/state；每个提交必须先补真实合同、再机械拆分，并重复超长函数库存、Python 3.9 AST、全量 Python、QML probe、headless CTest、changed scanner、用户配置/缓存隔离与 `git diff --check`。未使用导入与文件头仍按原计划后置，不得混入长函数提交。
...
| P7 Python 规范债务 | 进行中 | 已完成框架签名/生成枚举例外定义、异常边界前四批、P7E 配置事务/schema/测试隔离、P7F DPI/WindowType 严格合同、P7G QRCode QML URL 传输协议、P7H NativeWindow 状态事务，以及 P7I-A ShadowManager、P7I-B WindowCore、P7I-C SqlListModel、P7I-D1 logger、P7I-D2 WindowCompat、P7I-D3 WindowBuilder、P7I-D4 SplashBuilder、P7I-D5 PageManager traceback 边界。P7I-F 的 F1 前置 userCard 清理与 F1a 纯 QML 组合抽离已完成；F1a 当前门禁为 helper `6 passed`、真实 composition/fallback 退出 0、headless 入口 `22 passed`、Python `891 passed / 1 skipped`、changed QML `0`、QML `169 OK / 0 错误 / 12 跳过`、C++ headless `8/8`，全部 runner 零可见窗口/零残留进程，用户配置/缓存/audit 指纹未变化；本批未复用较早用户桌面确认。`_create_window()` 已从 223 行降至 154 行，但超长函数库存仍为 50。下一批为 F1b Engine/context 注入拆分；F1c、P7J-P7K 及剩余导入、文件头、benchmark 路径与测试 stdout 政策仍待处理 | `8272d2c0`、`354777fe`、`a8e80e86`、`c962014e`、`bb81d37b`、`e344c8a0`、`e48bb127`、`dafa2a34`、`1512f723`、`8e24b825`、`5fb9e7d8`、`620da260`、`77d045c5`、`cb19eb88`、`fe7a4f4e`、`21211777`、`3b717770`、`f2b4af11`、`cc4567b2`、`ef07928d`、`6db88a88` |
