...
| P7 Python 规范债务 | 进行中 | 已完成框架签名/生成枚举例外定义与异常边界前三批：元数据缺包回退收窄，DWM native callback 不吞 process-control 异常，logger.exception 真实输出 traceback，同一 LogRecord 跨 handler 仅输出一次堆栈；Store watcher 失败保留 traceback 并继续通知链；DWM filter 安装失败不再残留假单例，可重试且不污染 process-control 异常状态。Python 609/1、QML 169/0/12、CTest 6/6、Build All 七项全绿。剩余宽捕获、长函数、导入与文件头继续处理 | `8272d2c0`、`354777fe`、`a8e80e86`、`c962014e`、`bb81d37b` |
